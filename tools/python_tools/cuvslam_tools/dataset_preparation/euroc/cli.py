...
"""Installed console-script wrapper for the EuRoC preparation flow."""
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory for raw EuRoC archives. In a source checkout, defaults to the wrapped script default. "
            "In an installed package, defaults to ./datasets/euroc/raw."
        ),
    )
    parser.add_argument(
        "--output-dir",
    ...
    if not is_source_checkout:
        raw_dir = raw_dir or installed_raw_dir("euroc")
